...
from uploader.bilibili_uploader.extra import (
    get_bilibili_login_account_ids,
    get_bilibili_login_info,
    request_login_url,
)
from uploader.douyin_uploader.extra import get_douyin_login_account_ids
from uploader.douyin_uploader.login import (
    douyin_login,
    douyin_login_get_status,
    douyin_login_verify_sms_code,
)
...
async def xhs_get_login_qrcode_blob(background_tasks: BackgroundTasks):
    qrcode_image_src = await xhs_login_client(
...
async def tencent_get_login_qrcode_blob(background_tasks: BackgroundTasks):
    ...
    return {
        "code": 0,
        "data": {"id": login_info["id"], "qr_code_image_src": login_info["blob"]},
        "message": "success",
    }
...
async def upload_video_by_url(
    upload_video_by_url_request: UploadVideoByUrlRequest,
    background_tasks: BackgroundTasks,
):

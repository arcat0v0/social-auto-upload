...
from utils.redis import (
    add_to_bilibili_login_list,
    get_all_bilibili_login_ids,
    get_bilibili_login,
    register_bilibili_login,
)
import qrcode
...
async def test_login_by_qrcode():
    with BiliBili("test") as bili:
        ...
        url = res["data"]["url"]
        qr = qrcode.QRCode(
            version=1, error_correction=qrcode.ERROR_CORRECT_L, box_size=10, border=1
        )
        qr.add_data(url)
        qr.make()
        qr.print_ascii()
...
def get_bilibili_login_info(id: str):
    ...
    if login_information is None:
        response = {"code": 1, "message": "Login information not found"}
        return response, 403

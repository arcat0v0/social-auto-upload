...
async def douyin_login(background_tasks: BackgroundTasks, browser: Browser):
    ...
    async with async_playwright() as playwright:
        ...
        async def handle_response(response: Response, future):
            if (
                r"https://sso.douyin.com/get_qrcode" in response.url
            ...
            try:
                for i in range(0, 180):
                    ...
                    verify_card = (
                        page.locator("div")
                        .filter(
                            has_text="身份验证为保证帐号安全，请完成身份验证接收短信验证发送短信验证"
                        )
                        .nth(2)
                    )
                    ...
                    login_status = get_douyin_login(generated_login_uuid_str).get(
                        "login_status"
                    )
                    ...
                    if (
                        ...
                        await page.get_by_role("spinbutton").fill(
                            get_douyin_login(generated_login_uuid_str)[
                                "sms_verify_code"
                            ]
                        )
                    ...
                    if await id_span.is_visible():
                        ...
                        pattern = r"\d+"
                        account_id = re.search(pattern, account_id_text).group()
